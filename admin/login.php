...
<html lang="en">
  ...
  <div style="margin-top: 30px;float:left;margin-left:50px;"><a href="#" class="btn btn-primary btn-lg disabled" role="button" aria-disabled="true">Admin</a></div>
      <?php
        ...
        if(isset($_POST['signin'])){
            ...
            if(count(fetchAll($query)) > 0){ //this is to catch unknown error.
                  foreach(fetchAll($query) as $row){
                    if($row['email']==$idno&&$row['password']==$password){
                        $_SESSION['login'] = true;
